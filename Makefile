...
IMAGE_NAME			=  $(REGISTRY)/${NAMESPACE}/$(DRIVER_NAME)
IMAGE_TAG			?= $(VERSION)
...
PLURAL_EXCEPTIONS 	+= SpyreClaimParameters:SpyreClaimParameters
...
.PHONY: helm
helm: $(LOCALBIN)
	test -s $(HELM) || curl -fsSL https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | HELM_INSTALL_DIR=$(LOCALBIN) sh -s -- -v $(HELM_VERSION)
...
.PHONY: add-license
add-license: $(ADDLICENSE) ## Download add-license
$(ADDLICENSE): $(LOCALBIN)
...
.PHONY: vendor
vendor: ## Run go mod vendor command
	go mod vendor
...
.PHONY: license
license: add-license ## Add license header to go files
	find . -name "*.go" -type f \
		-not -path '*/.*' \
		-not -path '*/vendor/*' \
		-exec $(ADDLICENSE) -c "IBM Corp." -l apache {} +
...
docker-build-power: vendor ## Build the spyre dra driver image for linux/ppc64le
ifeq ($(DOCKER), docker)
...
docker-build-amd64: vendor ## Build the spyre dra driver image for linux/amd64
ifeq ($(DOCKER), docker)
...
docker-build-s390x: vendor ## Build the spyre dra driver image for linux/s390x
ifeq ($(DOCKER), docker)
...
docker-push-power: ## Push spyre dra driver image for ppc64le only
ifeq ($(DOCKER), docker)
...
docker-push-s390x: ## Push spyre dra driver for s390x only
ifeq ($(DOCKER), docker)
	echo "Images already pushed by Docker"
else
...
docker-push-amd64: ## Push dra driver for amd64 only
ifeq ($(DOCKER), docker)
	echo "Images already pushed by Docker"
else
...
docker-build-manifest: ## Build spyre dra driver manifest for all architectures
ifeq ($(DOCKER), docker)
...
docker-push-manifest: ## Push dra driver image multi architecture manifest
	$(DOCKER) manifest push $(IMAGE)

.PHONY: docker-buildx
docker-buildx: docker-build-power docker-build-amd64 docker-build-s390x ## Build dra driver image for all architectures

.PHONY: docker-pushx ## Push spyre dra driver image image for all architectures
