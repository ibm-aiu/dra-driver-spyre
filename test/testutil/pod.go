...
// parses senlib config, and checks DOOM mode enabled (GENERAL.doom).
//
...
func CheckAndGetAllocationsFromPodLog(ctx context.Context, k8sClientset *kubernetes.Clientset, name, namespace string) []string {
	...
	Expect(senlibConfig.General.Doom).To(BeTrueBecause("DOOM must be enabled in GENERAL config"))
