...
* [kind v0.17.0+](https://kind.sigs.k8s.io/docs/user/quick-start/) or [openshift v4.18+](https://docs.openshift.com/container-platform/4.18/release_notes/ocp-4-18-release-notes.html)
...
* **Kubernetes 1.33** with DynamicResourceAllocation [Feature Gates](https://kubernetes.io/docs/reference/command-line-tools-reference/feature-gates/) and CDI enabled.
  ...
  Check [example kind configuration](./demo/scripts/kind-cluster-config.yaml).
* **OpenShift 4.20.1**
...
2. Prepare cluster
    ...
    For testing on kind cluster, create a `kind` cluster to run it in:

    ```bash
    ./demo/create-cluster.sh
    ```

    For testing on existing `OpenShift` cluster, create secret

    ```bash
    kubectl create namespace dra-driver-spyre --dry-run=client -o yaml | kubectl apply -f - # create namespace if needed
    kubectl create secret docker-registry swg-devops-artifactory-aiu --from-file=.dockerconfigjson=./secret/auth.json -n dra-driver-spyre
    ```
...
3. Install the example resource driver via `helm`.


    > For testing on kind cluster, need to run `./demo/scripts/load-driver-image-into-kind.sh` to load image to newly created kind cluster first.

    ```
    make propagate-version
    ...
    helm upgrade -i \
      ...
      dra-driver-spyre \
      deployments/helm/dra-driver-spyre
    ```
...
4. Deploy examples and check results
    ...
    {"GENERAL":{"sen_bus_id":["0000:3d:00.0"],"target":"SOC"},"METRICS":{"general":{"enable":false,"path":"./sentientmap","port":8082}},"RISCV":{"DOOM":{"enable":false}},"SNT_MCI":{"DCR":{"MCI_CTRL":{"ENABLE_RISCV":"0x0"}}}}

    # kubectl exec -it pod1 -n spyre-test1 -- bash
    > root@pod1:/# cat /etc/aiu/senlib_config.json
    {"GENERAL":{"sen_bus_id":["0000:1e:00.0"],"target":"SOC"},"METRICS":{"general":{"enable":false,"path":"./sentientmap","port":8082}},"RISCV":{"DOOM":{"enable":false}},"SNT_MCI":{"DCR":{"MCI_CTRL":{"ENABLE_RISCV":"0x0"}}}}
    ```
    ...
    {"GENERAL":{"sen_bus_id":["0000:1a:00.0"],"target":"SOC"},"METRICS":{"general":{"enable":false,"path":"./sentientmap","port":8082}},"RISCV":{"DOOM":{"enable":false}},"SNT_MCI":{"DCR":{"MCI_CTRL":{"ENABLE_RISCV":"0x0"}}}}
    ```
  ...
  ```bash
    ...
    {"GENERAL":{"multi_aiu_config_path":"/tmp/testing","sen_bus_id":["0000:1e:00.0","0000:1a:00.0"],"target":"SOC"},"METRICS":{"general":{"enable":false,"path":"./sentientmap","port":8082}},"RISCV":{"DOOM":{"enable":false}},"SNT_MCI":{"DCR":{"MCI_CTRL":{"ENABLE_RISCV":"0x0"}}}}
    # kubectl exec -n spyre-test3 pod1 -- cat /etc/aiu/senlib_config.json
    {"GENERAL":{"multi_aiu_config_path":"/tmp/testing","sen_bus_id":["0000:3d:00.0","0000:3f:00.0","0000:40:00.0"],"target":"SOC"},"METRICS":{"general":{"enable":false,"path":"./sentientmap","port":8082}},"RISCV":{"DOOM":{"enable":false}},"SNT_MCI":{"DCR":{"MCI_CTRL":{"ENABLE_RISCV":"0x0"}}}}
  ```
